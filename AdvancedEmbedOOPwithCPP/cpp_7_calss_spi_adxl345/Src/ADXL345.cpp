...
#define ADXL345_SCALE_FACTOR		0.0078f
...
void ADXL345::updateAcceleration() {
	...
	// 从数据寄存器读取加速度计数据
	readRegister(ADXL345_REG_DATA_START, data_buffer, 1);
...
float ADXL345::getAccelXG() {
	return accel_x * ADXL345_SCALE_FACTOR;
}

// 获取 y 轴的原始加速度 g 单位下的值
float ADXL345::getAccelYG() {
	return accel_y * ADXL345_SCALE_FACTOR;
}

// 获取 z 轴的原始加速度 g 单位下的值
float ADXL345::getAccelZG() {
	return accel_z * ADXL345_SCALE_FACTOR;
}
